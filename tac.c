...
void generateTAC(ASTNode *node) {
  ...
  case NODE_BOOL_DECL: {
    ...
    TACInstr *instr = createTAC(TAC_DECL, "bool", NULL, node->data.name);
    instr->resultType = TYPE_BOOL;
    appendTAC(instr);
    break;
  }
  ...
  case NODE_ARRAY_DECL: {
    ...
    TACInstr *instr = createTAC(TAC_ARRAY_DECL, (char *)typeStr, sizeBuf,
...
void optimizeTAC() {
  ...
  if (hasControlFlow) {
    while (curr) {
      TACInstr *copy =
          createTAC(curr->op, curr->arg1, curr->arg2, curr->result);
      copyTypeInfo(copy, curr);
      copy->paramCount = curr->paramCount;
